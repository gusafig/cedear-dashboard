...
CEDEARS = {
    ...
    "BRK-B": {"name": "Berkshire Hathaway (Cl. B)",                "sector": "finance"},
    "O":     {"name": "Realty Income",                             "sector": "finance"},
    "PLD":   {"name": "Prologis",                                  "sector": "finance"},
    "WELL":  {"name": "Welltower",                                 "sector": "finance"},
    ...
    "HIMS":  {"name": "Hims & Hers Health",                        "sector": "consumer"},
    ...
    "PKX":   {"name": "POSCO Holdings",                            "sector": "industrial"},
    "CAAP":  {"name": "Corporación América Airports",              "sector": "industrial"},
    "TRV":   {"name": "The Travelers Companies",                   "sector": "industrial"},
    "XRX":   {"name": "Xerox Holding",                             "sector": "industrial"},
    ...
    "LAAC":  {"name": "Lithium Americas (Argentina)",              "sector": "mining"},
    "B":     {"name": "Barrick Mining Corp",                       "sector": "mining"},
    "NG":    {"name": "Novagold Resources",                        "sector": "mining"},
    "BIOX":  {"name": "Bioceres Crop Solutions",                   "sector": "brasil"},
    "TEN":   {"name": "Tenaris",                                   "sector": "brasil"},
    "AKO.B": {"name": "Embotelladora Andina",                      "sector": "brasil"},
    "LND":   {"name": "Brasilagro",                                "sector": "brasil"},
    "TX":    {"name": "Ternium",                                   "sector": "brasil"},
    "KOF":   {"name": "Coca-Cola FEMSA",                           "sector": "mexico"},
...
def compute_ema_series(data, period):
    """Calcula la serie completa de EMA."""
    if len(data) < period:
        return []
    k = 2 / (period + 1)
...
def compute_macd(closes, fast=12, slow=26, signal=9):
    """MACD correcto (12/26/9) con series de EMA."""
...
def download_batch(symbols, start_str, end_str, retries=3):
    ...
    for attempt in range(retries):
        try:
            raw = yf.download(
                tickers_str, start=start_str, end=end_str,
                interval="1d", auto_adjust=True,
                progress=False, threads=True, group_by="ticker",
            )
            if raw is None or raw.empty:
                print(f"    [RETRY {attempt+1}] lote vacío")
            else:
                out = {}
